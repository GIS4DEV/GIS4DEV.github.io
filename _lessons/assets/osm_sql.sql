...
/* Who is making the most edits to Dar es Salaam OSM data, and do you think that local knowledge figures prominently in this data? */ 

/* Let's try to find all of the universities in Dar es Salaam, and count how many pubs are found within 800 meters
Start by searching for unique values in the amenity field of the polygons and points tables. */

-- enter queries here. It's similar to counting how many features each OSM user edited.

/* Then, create new tables named unipoly and unipoint in your own schema containing the osm_id, way, and name for each point university feature and each polygon university features, using the proper UTM Zone
NOTE: by default, new tables are created in your own schema, and queries look for tables in your own schema first and in the public schema second. If you need to specify which schema to query or create a table in, add the schema name and a period before the table name. E.g. use dumb.unipoint to find the unipoint table in dumb schema or use public.planet_osm_line to find the planet_osm_line table in the public schema. */
...
-- Repeat for the polygons. The ILIKE text string comparison operator is simlar to =, but allows for differences in capitalization
...
/* ST_Dump seperates all the parts of a geometry into separate features. It's documented here: https://postgis.net/docs/ST_Dump.html 
I'm using this following the documentation, and do not know exactly why there is an extra set of parenthesis around st_dump folled by ".geom" which is like using AS to rename the output. I presume this is because st_dump() returns a set of rows, rather than just one. */

/* Now we have single-part buffers! We can select by location for any of the pubs intersecting our buffers, remembering to keep the coordinate refrence systems consistent in the comparison. Transform the smaller set of features (buffers) to match the larger set (planet_osm_polygon).
Remember that 'DISTINCT' avoids duplicates in case a pub was close to two or more university features */
...
/* SQL is smart: it will solve the amenity = pub part of this first. What if we crafted this to require comparing all the spatial information first? The following query took 46 seconds instead of the 2.7 seconds for the query above  */

SELECT DISTINCT osm_id, name 
FROM planet_osm_polygon INNER JOIN unibuffers 
ON st_intersects(planet_osm_polygon.way, st_transform(unibuffers.geom,4326));

/* meanwhile, the following query was taking so long that I just cancelled it... why do you think the query below is so much more inefficient than the query above? */
